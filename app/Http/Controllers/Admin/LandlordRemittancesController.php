...
class LandlordRemittancesController extends Controller
{
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.read_landlord_remittance')), 403);
        if (request()->ajax()) {
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.create_landlord_remittance')), 403);
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.delete_landlord_remittance')), 403);
