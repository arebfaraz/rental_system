...
class OverpaymentsController extends Controller
{
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.read_overpayment')), 403);
        if (\request()->ajax()) {
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.delete_overpayment')), 403);
