...
class LandlordsController extends Controller
{
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.read_landlord')), 403);
        if (\request()->ajax()) {
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.create_landlord')), 403);
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.update_landlord')), 403);
        $landlord = User::query()
            ->withoutTrashed()
            ->role('landlord')
            ->findOrFail($id);
