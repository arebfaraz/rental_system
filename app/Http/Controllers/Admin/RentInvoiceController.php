...
class RentInvoiceController extends Controller
{
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.read_rent_invoice')), 403);
        if (\request()->ajax()) {
...
//                ->setRowClass('nk-tb-item')
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.update_rent_invoice')), 403);
