...
class CustomInvoiceController extends Controller
{
    ...
    public function index()
    {
        ...
        abort_unless(auth()->user()->can(config('permission.permissions.read_custom_invoice')), 403);
        if (\request()->ajax()) {
    ...
    {
        abort_unless(auth()->user()->can(config('permission.permissions.create_custom_invoice')), 403);
    ...
    public function destroy($id)
    {
        ...
        abort_unless(auth()->user()->can(config('permission.permissions.delete_custom_invoice')), 403);
        $invoice = CustomInvoice::query()
            ->with('landlord')
            ->withSum('items', 'cost')
            ->findOrFail($id);
